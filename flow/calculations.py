...
def calc_vn4(ref, dif):
    ...
    c2 = np.divide(ref[..., rW2Two], ref[..., rW2], out=np.zeros_like(ref[..., rW2]), where=ref[..., rW2] > 0)
...
def calc_rnn_backward(ref, dif):
    ...
    rnn_av = np.zeros((10, 3, 7, 9))
    rnn_av[..., 0] = np.nanmean(rnn[..., 0:5], axis=3)
    rnn_av[..., 1] = np.nanmean(rnn[..., 5:10], axis=3)
    for c in range(2, 9):
        rnn_av[..., c] = np.nanmean(rnn[..., ((c - 1) * 10):(c * 10)], axis=3)
    return np.nanmean(rnn_av, axis=0)


def calc_rnn_forward(ref, dif):
    N = np.divide(dif[:, :, 21:, :, dW2TwoB], dif[:, :, 21:, :, dW2B], where=dif[:, :, 21:, :, dW2B] > 0,
                  out=np.full_like(dif[:, :, 21:, :, dW2B], float('NaN')))
    D = np.divide(dif[:, :, 21:, :, dW2TwoA], dif[:, :, 21:, :, dW2A], where=dif[:, :, 21:, :, dW2A] > 0,
                  out=np.full_like(dif[:, :, 21:, :, dW2A], float('NaN')))
    rnn = np.divide(N, D, where=D != 0, out=np.full_like(D, float('NaN')))

    rnn_av = np.zeros((10, 3, 13, 9))
    rnn_av[..., 0] = np.nanmean(rnn[..., 0:5], axis=3)
    rnn_av[..., 1] = np.nanmean(rnn[..., 5:10], axis=3)
    for c in range(2, 9):
        rnn_av[..., c] = np.nanmean(rnn[..., ((c - 1) * 10):(c * 10)], axis=3)
