...
def makeSumPlot(Zinit, mydata, detector):
    ...
    myplot(Zsum, mydata + '/' + savename + '/nua_total', '', r'$\mathrm{d}N_{ch} / \mathrm{d}\eta \mathrm{d}\varphi$',
           False, -1, detector)
    plt.show()
...
def makeCentralPlots(awesome, Zinit, mydata, cendetector, doedges, doplt):
    ...
    if doplt:
        for Zbin in range(0, zbins):
            myplot(Znorm[:, :, Zbin], mydata + '/central/nua_norm', '', r'$dN / d\varphi d\eta$', False, Zbin,
                   cendetector)
    ...
    for Zbin in range(0, zbins):  # 10
        ...
        for etabin in range(0, len(Znorm[:, 0, 0])):
            ...
            if doedges:
                ...
                #     Znorm[etabin,:,Zbin] = np.nan;
        if doplt:
            myplot(Znorm[:, :, Zbin], mydata + '/central/nua_norm_wfill', 'w. cuts', r'$dN_{ch} / d\eta d\varphi$',
                   False, Zbin, cendetector)
    ...
    if doplt:
        for Zbin in range(0, 10):
            myplot(Zmean[:, :, Zbin], mydata + '/central/nua_mean_wfill', '', r'$\langle N_{ch} \rangle / d\eta$',
                   False, Zbin, cendetector)

            myplot(Znew[:, :, Zbin], mydata + '/central/nua_frac', '', r'$\langle N \rangle / N$', False, Zbin,
                   cendetector)
        plt.show()
    Zcentral = Znew
    return Zcentral
...
def makeForwardPlots(awesome, Zinit, mydata, doedges, dofill):
    ...
    for Zbin in range(0, 10):  # 10
        ...
        myplot(Znorm[:, :, Zbin], mydata + '/forward/nua_norm_wfill', 'w. fills', r'$dN_{ch} / d\eta d\varphi$', False,
               Zbin, 'FMD')
    ...
    for Zbin in range(0, 10):
        myplot(Zmean[:, :, Zbin], mydata + '/forward/nua_mean_wfill', '', r'$\langle N_{ch} \rangle / d\eta$', False,
               Zbin, 'FMD')
...
def makeCentralPlotsSum(awesome, Zinit, mydata, doedges, doplt, detector):
    ...
    if doplt:
        myplot(Znorm[:, :], mydata + '/central/nua_norm_wfill', 'w. cuts', r'$dN_{ch} / d\eta d\varphi$', False, -1,
               detector)
    ...
    Zmean[Zmean == 0] = np.nan
    if doplt:
        myplot(Zmean[:, :], mydata + '/central/nua_mean_wfill', '', r'$\langle N_{ch} \rangle / d\eta$', False, -1,
               detector)
...
def nua_per_run(run_list, title, save_dir, input_file, branch, forward, y_low, y_up, yx_low, yx_up):
    hist_list = []
    colors = sns.color_palette("Paired", 20)

    if forward:
        xlist = gen.make_list(0, 2 * np.pi, 20)
        histname = 'NUA_fwd'
        detector = 'FMD'
    else:
        xlist = gen.make_list(0, 2 * np.pi, 300)
        histname = 'NUA_cen'
        detector = 'TPC'
    ...
    for i in range(0, len(run_list)):
        input = input_file + 'output/' + run_list[i] + '.root'
        datafile = root_open(input, 'read')

        awesome = datafile.V0M_globaltracks
